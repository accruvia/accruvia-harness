    ...
    def build_prompt(self, project: Project, context: dict[str, Any]) -> str:
        return "\n\n".join(
            [
                ...
                "Once recent runs show that a previously dominant reliability problem is no longer recurring, stop over-prioritizing it. Re-open the search space to product direction, UX, DX, observability, and other high-value work that was previously deferred for reliability reasons.",
                "Healthy idle is not completion. If loop_status says healthy_idle and queue depth is 0, treat that as a cue to choose the next highest-value bounded work, not as evidence that the project is done.",
                "In healthy idle with an empty queue, propose 1-3 concrete tasks unless the project is explicitly complete, archived, or blocked on an external decision that no local task can unblock.",
                "When you do choose zero tasks during healthy idle, explain the blocking reason concretely and do not recommend more than 1800 seconds before the next heartbeat.",
                ...
                "When proposing operator UX/DX/observability work, prefer validation_mode=lightweight_operator so small CLI or status improvements do not get trapped behind broad validation suites.",
